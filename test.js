...
test('readSync ignoreEmptyDirs false', function (t) {
    rimraf.sync('testdir.tmp');
    fs.mkdirSync('testdir.tmp');
...
test('readSync ignoreEmptyDirs true', function (t) {
    rimraf.sync('testdir.tmp');
    fs.mkdirSync('testdir.tmp');
    fs.mkdirSync('testdir.tmp/emptydir');
    t.deepEqual(fixturify.readSync('testdir.tmp', {
...
test('error conditions', function (t) {
    test('writeSync arguments requires specific input', function (t) {
        // @ts-ignore
        t.throws(function () { return fixturify.writeSync(); });
        // @ts-ignore
        t.throws(function () { return fixturify.writeSync(null); });
        // @ts-ignore
        t.throws(function () { return fixturify.writeSync(null, null); });
        // @ts-ignore
        t.throws(function () { return fixturify.writeSync(null, {}); });
        t.end();
    });
    test('writeSync guards against misuse that could cause data loss', function (t) {
        ...
        rimraf.sync('testdir.tmp');
        t.throws(function () { return fixturify.writeSync('', {}); }, /non-empty string/);
        t.throws(function () { return fixturify.writeSync('testdir.tmp', { '': 'contents' }); }, /non-empty string/);
        t.throws(function () { return fixturify.writeSync('testdir.tmp', { '.': {} }); }, /must not be "\." or "\.\."/);
        t.throws(function () { return fixturify.writeSync('testdir.tmp', { '..': {} }); }, /must not be "\." or "\.\."/);
        t.throws(function () { return fixturify.writeSync('testdir.tmp', { 'foo/bar': {} }); }, /must not contain "\/" or "\\"/);
        t.throws(function () { return fixturify.writeSync('testdir.tmp', { 'foo\\bar': {} }); }, /must not contain "\/" or "\\"/);
        t.end();
    });
    ...
    test('readSync throws on broken symlinks', function (t) {
        ...
        t.throws(function () { return fixturify.readSync('testdir.tmp'); });
        rimraf.sync('testdir.tmp');
        t.end();
    });
    t.end();
